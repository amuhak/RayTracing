...
void camera::render(const hittable &world, const size_t threads) {
    ...
    if constexpr (USE_DISPLAY) {
        disp.emplace();
        display_thread.emplace(&display::run, &disp.value(), static_cast<uint32_t>(image_width),
                               static_cast<uint32_t>(image_height), std::ref(img.data));
    }
...
void camera::initialize() {
    ...
    image_height =
            std::max(static_cast<size_t>(1), static_cast<size_t>(static_cast<double>(image_width) / aspect_ratio));
...
[[nodiscard]] vec3 camera::sample_square() { return {random_double() - 0.5, random_double() - 0.5, 0}; }
